...
This PEP proposes adding a pattern matching statement [1]_ to Python in
order to create more expressive ways of handling heterogeneous structured
data.

Python already provides some form of destructuring assignment (as defined
in :pep:`3132` and :pep:`448`) which works well on nested iterable structures.
However it's also useful to be able to to also extract components from nested
sequences, mappings, and arbitrary object attributes. When decomposing
structures, it's also useful to be able to recognize between multiple possible
shapes of the data.

This PEP introduces "patterns", which generalize unpacking to be able to extract
components from nested structures. It adds a new ``match`` compound statement that
is able to recognize if one of multiple patterns correspond to the structure of
an object (both regarding its type and its components) and take an associated
branch of the statement.

A side-effect of this approach is that it allows a functionality similar to
a ``switch`` statement as used in C or Javascript, and proposed by rejected
:pep:`275` and :pep:`3103`. That similarity can be useful but is not the main
goal of this proposal, and this design
focuses instead on generalizing sequence, mapping, and object destructuring.


Informal Overview
=================

This section gives an informal presentation of the new syntax and how to read
it. It can be informative if you're unfamiliar with the feature, although the
description here is not intended to be perfectly accurate.

A new syntactic construct called *pattern* is introduced. Syntactically 
patterns look like a subset of expressions; the following are patterns:
...
If the above were interpreted as expressions, they could be read as object
construction. The meaning of a pattern is actually the reverse, object
deconstruction; when matched to an object, a pattern sets the free variables
in the pattern that would result in the desired value. For example, matching
``Point2d(x, 0)`` to a value of ``Point2d(3, 0)`` successfully matches and
binds ``x`` to 3. A match could also fail, meaning that there are no values of
the variables

In short a pattern tries to answer the question "Are there values for my
variables be set that would make me produce the matched object?", and if the
answer is "yes", the pattern assigns to those variables. In the example above,
setting ``x`` to 3 would make the pattern produce the value ``Point2d(3, 0)``,
and that's why the matching succeeds. Note however that no actual object is
constructed, in the same way that a Python assignment ``[a, b] = my_list``
doesn't create a new ``[a, b]`` list.

These patterns are used in a new statement called ``match``. This statement
tries to match multiple patterns in order to a single object, and for the
first successful match, the variables in the pattern are assigned and a
corresponding block is executed. Each one of the multiple branches of this
conditional statement can also have a boolean condition. Here's an example
use of a match statement, used to define a function building 3d points that can
accept as input either tuples of size 2 or 3, or existing points::
...
Writing this function in the traditional fashion would require multiple 
``isinstance()`` checks, a pair of ``len()`` calls, and a more convoluted
control flow. While the ``match`` version translates into similar code under
the hood, to a reader familiar with patterns it is much clearer.


Rationale and Goals
===================

Software often defines operations on nested data structures.
The success of data exchange formats like JSON, XML, YAML which are based on
nested data structures is a consequence, not a cause of
that. These structures are made of objects, mapping, sequences and end up
with simpler types likes numbers, strings, or dates. Frequently, some
components of those structures are heterogeneous, not necessarily in the
sense of OO polymorphism, but in the sense that they could have multiple types
without necessarily a common interface. 

When using polymorphic class instances in a purist OO approach, type selection
is done based on polymorphic method calls. However, this often leads towards
complex inheritance structures, and hard to follow logic spread over classes.
Many developers are opting for "dumb data" approaches where the logic is
outside the data structures. Python supports that designs allowing stand-alone
functions (instead of forcing everything to be a method) and structures like
:pep:`557` data classes. However, that forces our functions to handle
heterogeneous data, which requires some form of conditional statement.

Much of the logic operating on heterogenous data tends to follow
the same style:

* some verification is done on the "shape" of the data: This could involve 
  ``isinstance()`` calls and/or extracting components (via indexing or attribute
  access) which are checked for specific values or conditions.
* if the "shape" is the expected one, possible some more components are
  extracted and some operation is done using the extracted values.

Take for example this piece of the Django web framework::


    # https://github.com/django/django/blob/5166097d7c80cab757e44f2d02f3d148fbbc2ff6/django/db/models/enums.py#L13
    if (
...
destructuring inside. Note that shape here involves checking the types both
of the container and of one of its components, and some checks on its number
of elements. Once we match the shape, we need to decompose the sequence.
With the proposal in this PEP, you could rewrite that code into this::
...
This syntax makes much more explicit what formats are possible for the input
data, and which components are extracted from where. You can see a pattern
similar to list unpacking, but also type checking: the ``Promise()`` `pattern`
is not an object construction, but represents anything that's an instance of
``Promise``. The pattern operator ``|`` represent alternatives (not unlike
regular expressions), and the ``_`` is used as a wildcard.

In some occasions, extraction of information is not as relevant as identifying
structure. Take the following example from the standard library::
...
This example shows an example of finding out the "shape" of the data, without
doing significant extraction. This code is not very easy to read, and the
intended shape that this is trying to match is not evident. Compare with the
updated code using this document's syntax::
...
definition of ``Node`` and other classes here. As shown in the examples above
the proposal supports not just unpacking sequences, but also doing
``isinstance`` checks (like ``LParen()`` or
``str()``), looking into object attributes (``Leaf(value="(")`` is an example)
and comparing with literals.

That last feature helps writing some kinds of code which look more similar to
a "switch" statement as present in other languages::
...
Although this will be valid code, it's not necessary what the proposal is
focused on, so its syntax has been designed to best support the destructuring
scenarios.
...
all. This is implemented using syntactic features made possible by 
the new parser introduced in :pep:`617`.
